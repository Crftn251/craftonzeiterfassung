-- Drop the existing view and recreate it with security filtering
DROP VIEW IF EXISTS public.v_time_entries_formatted;

-- Recreate the view with built-in security filtering
...
SELECT 
  ...
  CASE 
    WHEN te.ended_at IS NOT NULL THEN
      LPAD(((EXTRACT(EPOCH FROM (te.ended_at - te.started_at))::integer - COALESCE(te.paused_seconds, 0)) / 3600)::text, 2, '0') || ':' ||
      LPAD((((EXTRACT(EPOCH FROM (te.ended_at - te.started_at))::integer - COALESCE(te.paused_seconds, 0)) % 3600) / 60)::text, 2, '0')
    ELSE '00:00'
  END as net_hhmm,
