...
-- Profiles policies
create policy if not exists "Profiles: users can select their own or admins can select all"
  on public.profiles
  for select
  to authenticated
  using (auth.uid() = id or public.is_admin(auth.uid()));

create policy if not exists "Profiles: users can insert their own row"
  on public.profiles
  for insert
  to authenticated
  with check (auth.uid() = id);

create policy if not exists "Profiles: users can update their own or admins any"
  on public.profiles
  for update
  to authenticated
  using (auth.uid() = id or public.is_admin(auth.uid()))
...
create policy if not exists "Profiles: only admins can delete"
  on public.profiles
  for delete
  to authenticated
...
create policy if not exists "Branches: anyone authenticated can read"
  on public.branches
  for select
  to authenticated
  using (true);

create policy if not exists "Branches: only admins can insert"
  on public.branches
  for insert
  to authenticated
  with check (public.is_admin(auth.uid()));

create policy if not exists "Branches: only admins can update"
  on public.branches
  for update
  to authenticated
  using (public.is_admin(auth.uid()))
...
create policy if not exists "Branches: only admins can delete"
  on public.branches
  for delete
  to authenticated
  using (public.is_admin(auth.uid()));
...
create policy if not exists "Activities: anyone authenticated can read"
  on public.activities
  for select
  to authenticated
  using (true);

create policy if not exists "Activities: only admins can insert"
  on public.activities
  for insert
  to authenticated
  with check (public.is_admin(auth.uid()));

create policy if not exists "Activities: only admins can update"
  on public.activities
  for update
  to authenticated
  using (public.is_admin(auth.uid()))
...
create policy if not exists "Activities: only admins can delete"
  on public.activities
  for delete
  to authenticated
  using (public.is_admin(auth.uid()));
...
create policy if not exists "Time entries: users can read their own"
  on public.time_entries
  for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Time entries: users can insert their own"
  on public.time_entries
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Time entries: users can update their own"
  on public.time_entries
  for update
  to authenticated
  using (auth.uid() = user_id)
...
create policy if not exists "Time entries: users can delete their own"
  on public.time_entries
  for delete
  to authenticated
  using (auth.uid() = user_id);
